...
import { clearSession, getSessionData } from "@/utils/session";
import { IUserProfile } from "@/types";
import ClickOutside from "@/components/ClickOutside";
import { showErrorAlert, showSuccessAlert } from "@/utils/alert";
import { postLoginRequest } from "@/services/api.service";
// import { SIGNOUT } from "@/utils/apiMessage";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        showSuccessAlert(
          "Logout Successful",
          "Please login again access your account",
        );
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (details) {
          ...
          setProfileImage(
            details?.profileImg.doc || "/images/user/profile.webp",
          );
        }
      } catch (error) {
