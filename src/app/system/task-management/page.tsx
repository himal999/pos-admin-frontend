...
import { IFilter, ITaskItem } from "@/types";
...
function Task() {
  ...
  const [task, setTask] = useState<IFilter>({
    page: 0,
    size: size,
  ...
  const handlePreviousPage = () => {
    if (task.page > 0) {
      const newPage = task.page - 1;
      setCurrentPage(newPage);
      setTask({ ...task, page: newPage });
    }
  };

  const handleNextPage = () => {
    const totalPages = Math.ceil(totalRecords / size);
    if (task.page < totalPages - 1) {
      const newPage = task.page + 1;
      setCurrentPage(newPage);
      setTask({ ...task, page: newPage });
    }
  ...
  const renderSortIcons = (column: string) => {
    const isActive = task.sortColumn === column;
    const direction = task.sortDirection;
    ...
    <DefaultLayout>
                  ...
                  >
                    <td className="w-[10%] px-6 py-4">
                      <div className="flex items-center justify-center gap-3">
                        {privileges.view && (
                          <button
                            onClick={() => handleViewtask(item.id.toString())}
                            title="View User"
                          >
                        ...
                        {privileges.update && (
                          <button
                            onClick={() => handleEdittask(item.id.toString())}
                            title="Update User"
                          >
      ...
      <div className="mt-4 flex items-center justify-between">
        <p className="text-sm text-gray-600">
          Page {task.page + 1} of {Math.ceil(totalRecords / size)}
        </p>
        <div className="flex gap-2">
          <button
            onClick={handlePreviousPage}
            disabled={task.page === 0}
          ...
          <button
            ...
            disabled={task.page >= Math.ceil(totalRecords / size) - 1}
            className={`rounded-lg border px-4 py-2 text-sm font-medium ${
              task.page >= Math.ceil(totalRecords / size) - 1
                ? "cursor-not-allowed bg-gray-300 text-gray-600"
                : "bg-white text-black hover:bg-gray-100"
            }`}
          >
      ...
      {showModal && selectedtask && (
        <div className="fixed inset-0 z-9999 flex items-center justify-center bg-black bg-opacity-40">
          <div className="relative m-2 w-full max-w-md rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:border dark:border-gray-500">
            ...
            <p className="mb-2">
              <strong>Code:</strong> {selectedtask.code}
            </p>
            <p className="mb-2">
            ...
            {modalMode === "edit" ? (
              ...
              <p className="mb-2">
                <strong>Status:</strong> {selectedtask.statusDescription}
              </p>
            )}
            <p className="mb-2">
              <strong>Created Date:</strong> {selectedtask.createdDate}
            </p>
            <p className="mb-2">
              <strong>Modified Date:</strong> {selectedtask.lastModifiedDate}
            </p>
            <p className="mb-2">
              <strong>Created By:</strong> {selectedtask.createdBy}
            </p>
            <p className="mb-2">
              <strong>Modified By:</strong> {selectedtask.lastModifiedBy}
            </p>
