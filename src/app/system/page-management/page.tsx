...
import { IFilter, IPageItem } from "@/types";
...
function ViewPage() {
  ...
  const [pageList, setPageList] = useState<IPageItem[]>([]);
  ...
  const [viewPage, setViewPage] = useState<IFilter>({
    page: 0,
    size: size,
  ...
  const handlePreviousPage = () => {
    if (viewPage.page > 0) {
      const newPage = viewPage.page - 1;
      setCurrentPage(newPage);
      setViewPage({ ...viewPage, page: newPage });
    }
  };

  const handleNextPage = () => {
    const totalPages = Math.ceil(totalRecords / size);
    if (viewPage.page < totalPages - 1) {
      const newPage = viewPage.page + 1;
      setCurrentPage(newPage);
      setViewPage({ ...viewPage, page: newPage });
    }
  ...
  const renderSortIcons = (column: string) => {
    const isActive = viewPage.sortColumn === column;
    const direction = viewPage.sortDirection;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Page Management" />
      {privileges.search && (
        <>
      ...
      <div className="mt-4 flex items-center justify-between">
        <p className="text-sm text-gray-600">
          Page {viewPage.page + 1} of {Math.ceil(totalRecords / size)}
        </p>
        <div className="flex gap-2">
          <button
            onClick={handlePreviousPage}
            disabled={viewPage.page === 0}
          ...
          <button
            ...
            disabled={viewPage.page >= Math.ceil(totalRecords / size) - 1}
            className={`rounded-lg border px-4 py-2 text-sm font-medium ${
              viewPage.page >= Math.ceil(totalRecords / size) - 1
                ? "cursor-not-allowed bg-gray-300 text-gray-600"
                : "bg-white text-black hover:bg-gray-100"
            }`}
          >
