...
import { IFilter, IBrand, IAddCategory } from "@/types";
import {
  ADD,
  DELETE,
  FILTERLIST,
...
function Brand() {
  ...
  const [newBrand, setNewBrand] = useState<IAddCategory>({
    code: "",
    description: "",
    status: "ACTIVE",
  });
  ...
  const fetchReferenceData = async () => {
    try {
      ...
      const response = await postLoginRequest(
        "api/v1/item-brand/reference-data",
        {},
        REFDATA,
        token,
        username,
  ...
  const handleViewBrand = async (id: string) => {
    try {
      const token = getSessionData("accessToken") || "";
      const username = getSessionData("userProfile")?.username || "";
      const response = await postLoginRequest(
        "api/v1/item-brand/view",
        { id },
        VIEW,
        token,
        username,
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Item Brand Management" />
                      ...
                      <td className="w-[10%] px-6 py-4">
                        <div className="flex items-center justify-center gap-3">
                          {privileges.view && (
                            <button
                              onClick={() =>
                                handleViewBrand(item.id.toString())
                              }
                              title="View User"
                            >
                          ...
                          {privileges.update && (
                            <button
                              onClick={() =>
                                handleEditBrand(item.id.toString())
                              }
                              title="Update User"
                            >
          ...
          <div className="fixed inset-0 z-9999 flex items-center justify-center bg-black bg-opacity-40">
            <div className="relative m-2 w-full max-w-md rounded-2xl bg-white p-6 text-gray-600 shadow-lg dark:border dark:border-gray-500 dark:bg-gray-800 dark:text-gray-300">
              ...
              <h2 className="mb-4 text-lg font-semibold">
                {modalMode === "view" ? "View Brand" : "Edit Brand"}
              </h2>

              <p className="mb-2">
                <strong>Code:</strong> {selectedBrand.code}
              </p>
              {modalMode === "edit" ? (
                <div className="mb-2">
                  <strong>Name:</strong>
              ...
              {modalMode === "edit" ? (
                ...
                <p className="mb-2">
                  <strong>Status:</strong> {selectedBrand.statusDescription}
                </p>
              )}
              <p className="mb-2">
                <strong>Created Date:</strong> {selectedBrand.createdDate}
              </p>
              <p className="mb-2">
                <strong>Modified Date:</strong> {selectedBrand.lastModifiedDate}
              </p>
              <p className="mb-2">
                <strong>Created By:</strong> {selectedBrand.createdBy}
              </p>
              <p className="mb-2">
                <strong>Modified By:</strong> {selectedBrand.lastModifiedBy}
              </p>
